...
foreach ($ips as $ip) {
    ...
    if (!$result) {
        ...
        die(
            '<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>'
        );
        exit();
    }
}
    ...
    private function ip_in_range($ip, $range)
    {
        if (strpos($range, '/') !== false) {
            ...
            if (strpos($netmask, '.') !== false) {
                ...
                return (ip2long($ip) & $dnetmask) ==
                    (ip2long($range) & $dnetmask);
            } else {
                $r = explode('.', $range);
                while (count($r) < 4) {
                    $r[] = '0';
            ...
            if (strpos($range, '-') !== false) {
                ...
                $dip = $this->toLongs($ip);
                return $this->compare($dip, $dlow) != -1 &&
                    $this->compare($dip, $dhigh) != 1;
            }
        }
        return $ip == $range;
    }
...
class IpBlockList
{
    ...
    public function __construct(
        $whitelistfile = './e/whitelist.dat',
        $blacklistfile = './e/blacklist.dat'
    ) {
    ...
    public function ipPass($ip)
    {
        ...
        if ($this->whitelist->is_inlist($ip)) {
            $retval = true;
            $this->message =
                $ip . " is whitelisted by " . $this->whitelist->message() . ".";
            $this->status = $this->statusid['positive'];
        } elseif ($this->blacklist->is_inlist($ip)) {
            $retval = false;
            $this->message =
                $ip . " is blacklisted by " . $this->blacklist->message() . ".";
